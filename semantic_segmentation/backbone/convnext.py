...
def layer_wise_rotation_offset(k, K, enable_layer_cycle=0):
    ...
    """
    if enable_layer_cycle: return (k%2)/2
    return 0
...
def get_convolution_1d_arbitrary_size(in_channels, out_channels, *, kernel_size, padding, groups, stride=1, N=8, layer_offset=0, nhwc=False, resid=False, bias=True):
    ...
    theta = [theta_offset(channel_cycle_offset(k, in_channels, N)+layer_offset) for k in range(in_channels)]
    conv = DepthwiseOrientedConv1d(in_channels=in_channels, out_channels=in_channels, groups=in_channels, stride=stride, 
                                    nhwc=nhwc, resid=resid, kernel_size=kernel_size, padding=padding, bias=bias, angle=theta,
                                    arbitrary_size=True)
    return Wrapper(conv)
...
class Stem1D(nn.Module):
    ...
    """
    def __init__(self, dim0, dim1, dim2, kernel_size, enable_layer_cycle=0):
        super().__init__()
        ...
        self.dw1 = get_convolution_1d_arbitrary_size(dim1, dim1, kernel_size=kernel_size, stride=stride, padding=padding, groups=dim1, 
                                    nhwc=True, resid=False, layer_offset=layer_wise_rotation_offset(0, 2, enable_layer_cycle))
        self.dw2 = get_convolution_1d_arbitrary_size(dim1, dim1, kernel_size=kernel_size, stride=stride, padding=padding, groups=dim1, 
                                    nhwc=True, resid=False, layer_offset=layer_wise_rotation_offset(1, 2, enable_layer_cycle))
...
class Block2D(nn.Module):
    ...
    Args:
        ...
        layer_offset (float): Layer-wise rotation offset. Default: 0.
    """
    def __init__(self, dim, kernel_sizes=[7], N=0, drop_path=0., layer_scale_init_value=1e-6, layer_offset=0):
        super().__init__()
        
        self.dwconv = nn.Conv2d(dim, dim, kernel_size=kernel_sizes[0], padding=kernel_sizes[0]//2, groups=dim)
...
class Block1D(nn.Module):
    ...
    Args:
        ...
        layer_offset (float): Layer-wise rotation offset. Default: 0.
    """
    def __init__(self, dim, kernel_sizes=[31], N=8, drop_path=0., layer_scale_init_value=1e-6, layer_offset=0):
        super().__init__()
        ...
        self.dwconv = get_convolution_1d_arbitrary_size(dim, dim, kernel_size=kernel_sizes[0], padding=kernel_sizes[0]//2, groups=dim, 
                                        nhwc=False, resid=False, N=N, layer_offset=layer_offset, bias=True)
...
class Block1DPP(nn.Module):
    ...
    Args:
        ...
        layer_offset (float): Layer-wise rotation offset. Default: 0.
    """
    def __init__(self, dim, kernel_sizes=[15,31], N=8, drop_path=0., layer_scale_init_value=1e-6, layer_offset=0):
        super().__init__()
        ...
        self.dwconv1 = get_convolution_1d_arbitrary_size(dim, dim, kernel_size=kernel_sizes[0], padding=kernel_sizes[0]//2, groups=dim, 
                                        nhwc=False, resid=False, N=N, layer_offset=layer_offset)
        self.dwconv2 = get_convolution_1d_arbitrary_size(4*dim, 4*dim, kernel_size=kernel_sizes[1], padding=kernel_sizes[1]//2, groups=4*dim, 
                                        nhwc=True, resid=True, N=N, layer_offset=layer_offset)
...
class Block2DPP(nn.Module):
    ...
    Args:
        ...
        layer_offset (float): Layer-wise rotation offset. Default: 0.
    """
    def __init__(self, dim, kernel_sizes=[7,31], N=8, drop_path=0., layer_scale_init_value=1e-6, layer_offset=0):
        super().__init__()
        ...
        self.dwconv1 = nn.Conv2d(dim, dim, kernel_size=kernel_sizes[0], padding=kernel_sizes[0]//2, groups=dim)
        self.dwconv2 = get_convolution_1d_arbitrary_size(4*dim, 4*dim, kernel_size=kernel_sizes[1], padding=kernel_sizes[1]//2, groups=4*dim, 
                                        nhwc=True, resid=True, N=N, layer_offset=layer_offset)
...
block_sizes = {
    '2d':   [[7],[7],[7],[7]],
    '2dpp': [[7,31],[7,31],[7,27],[7,13]],
    '1d':   [[31],[31],[27],[13]],
    '1dpp': [[15,31],[15,31],[15,27],[13,13]],
}
stem_sizes = {
    '2d': 4,
    '2dpp': 4,
...
class ConvNeXt(nn.Module):
    ...
    def __init__(self, in_chans=3, num_classes=1000, 
        ...
        if variant in ['2d', '2dpp']:
            ...
            stem = Stem1D(dim0=in_chans, dim1=stem_dim1, dim2=dims[0], kernel_size=kernel_size, enable_layer_cycle=enable_layer_cycle)
        ...
        for i in range(4):
            stage = nn.Sequential(
                *[Block(dim=dims[i], drop_path=dp_rates[cur + j], 
                    ...
                    layer_offset=layer_wise_rotation_offset(j, depths[i], enable_layer_cycle)) 
