...
import datetime
import numpy as np
from timm.utils import get_state_dict, AverageMeter
...
def visualize_erf(model, data_path, output_dir, num_images):
    #   ================================= transform: resize to 1024x1024
    #model = ERFWrapper(model)
    t = [
    ...
    root = os.path.join(data_path, 'imagenet/val')
    ...
    for i, (samples, _) in enumerate(data_loader_val):
        print(i)
        if meter.count*batch_size >= num_images:
            break
        ...
        if (meter.count*batch_size+1)%25 == 0:
            it = meter.count*batch_size+1
            print(f'i={it}/{num_images}')
            save_path = output_dir / f'erf_matrix_{it}.npy'
...
def save_model(args, epoch, model, model_without_ddp, optimizer, loss_scaler, name=None,model_ema=None):
...
def auto_load_model(args, model, model_without_ddp, optimizer, loss_scaler, model_ema=None, strict=True):
    output_dir = Path(args.output_dir)
    ...
    if args.resume:
        ...
        if hasattr(args, 'model_ema') and args.model_ema:
            if 'model_ema' in checkpoint.keys():
                print(f'Loading EMA weights into EMA: strict={strict}')
                model_ema.ema.load_state_dict(checkpoint['model_ema'], strict=strict)
            else:
                print('Loading non-EMA weights into EMA')
                model_ema.ema.load_state_dict(checkpoint['model'], strict=strict)
