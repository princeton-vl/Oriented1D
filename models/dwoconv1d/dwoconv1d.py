...
from dwoconv1d_kernel import get_dwoconv1d_kernel
...
def oriented_dwconv1d_get_module_from_params(ftype, kernel_size, stride, pad, Sb=None):
    module = get_dwoconv1d_kernel(ftype, kernel_size, stride, pad, Sb=Sb)
    return (
        module.fprop,
        module.dgrad,
        module.wgrad,
...
class DepthwiseOrientedConv1d(nn.Conv2d):
    def __init__(
        ...
        *,
        ...
        # Compile kernels
        self.module_fp32 = oriented_dwconv1d_get_module_from_params(
            "float", kernel_size, self.stride[0], pad, Sb=1
        )

        if arbitrary_size:
            self.forward = self.forward_safe
            self.module_fp32_large = oriented_dwconv1d_get_module_from_params(
                "float", kernel_size, self.stride[0], pad, Sb=4
            )
    ...
    def forward_safe(self, x):
        ...
        hpow2, wpow2 = min(next_power_of_2(h) // 2, 256), min(
            next_power_of_2(w) // 2, 256
        )
